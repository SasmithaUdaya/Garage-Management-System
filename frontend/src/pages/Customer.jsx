...
export default function Customer() {
  ...
  const confirmSignOut = async () => {
    ...
    try {
      dispatch(signOutCustomerStart())
      const res = await fetch('/backend/customer/signout');
      const data  = await res.json();
      if(data.success === false) {
        dispatch(signOutCustomerFailure(data.message));
        return;
      }
        dispatch(signOutCustomerSuccess(data));
    }catch (error){
      dispatch(signOutCustomerFailure(data.message));
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <input type="text" placeholder="username" defaultValue={currentUser.name} id='name'
        ...
        <input type="text" placeholder="mobile" defaultValue={currentUser.phone_number} id='phone_number'
          className="border p-3 rounded-lg" onChange={handleChange} />
        <input type="text" placeholder="address" defaultValue={currentUser.address} id='address'
          className="border p-3 rounded-lg" onChange={handleChange} />
